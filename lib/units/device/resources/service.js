...
module.exports = syrup.serial()
  ...
  .define(function(options, adb) {
    ...
    function getPath() {
      log.info('pm path %s', resource.pkg)
      return adb.shell(options.serial, ['pm', 'path', resource.pkg])
        ...
        .then(function(out) {
          return streamutil.findLine(out, (/^package:/))
            .timeout(15000)
            .then(function(line) {
              log.info('pm path --> %s', line.substr(8))
    ...
    function install() {
      log.info('Checking whether we need to install STFService by verifying sdk version')
      return adb.shell(options.serial, "getprop ro.build.version.sdk")
      .timeout(10000)
      .then(function(out) {
        return streamutil.readAll(out)
          .timeout(10000)
          .then(function(buffer) {
            var sdk_version = buffer.toString()
            log.info('sdk_version = %d', sdk_version)
            return getPath()
              ...
              .catch(function() {
              if (parseInt(sdk_version, 10) > 28) {
                log.info('Skip installing STFService.apk due to invalid sdk version')
                return sdk_version
              }
              else {
    ...
    return install()
      .then(function(path) {
        log.info('STFService up to date. path=%s', path)
        resource.path = path
        return resource
      })
  })
