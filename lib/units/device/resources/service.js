        ...
        .then(function(out) {
          streamutil.readAll(out)
            ...
            .then(function(buffer) {
              ...
              log.info('SDK#1 %s', sdk)
              var sdk = parseInt(sdk, 10)
              log.info('SDK#2 %d', sdk)

              if (sdk > 28) {
                log.info('STFService not installed due to wrong sdk version')
                return buffer.toString()
              }
              else {
                log.info('Checking path of STFService')
                  return getPath()
